...
def start_correct_date_start_stop():
    ...
    app.logger.info('*** Сегодня: {}, Завтра: {}, Интервал: {} дней, Конечная дата: {}, Найдено: {} туров'.format(
        date_now, date_start, SEARCH_INTERVAL_DAYS, date_stop, len(sql)))
    for tour in sql:
        ...
        try:
            db.session.commit()
            app.logger.info(f"Данные о туре {tour.showcase_id}/{tour.id} успешно обновлены")
        except exc.SQLAlchemyError as e:
            msg = f"При изменении тура {tour.showcase_id}/{tour.id} произошла ошибка"
...
def start_disable_failed_tours():
    ...
    app.logger.info('*** Сегодня: {}, не обрабатывать туры старше {} ({} дня назад) или больше чем {} ошибок'.format(
    ...
    for record in sql:
        ...
        app.logger.info("Showcase={}, Tour={}, Tour.errors={}, TourSearch.updated={}".format(
